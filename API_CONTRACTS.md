...
- сигнальные ключевые слова ищутся **только в сообщениях с `role=user`**; ответы ассистента намеренно исключены, чтобы предотвратить цикл: прошлый chart-ответ с `legend`/`axis` не должен триггерить chart-режим в следующем запросе.
