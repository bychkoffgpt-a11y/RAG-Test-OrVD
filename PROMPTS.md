...
- `v6` (2026-05-04): системная vision-инструкция в prompt-builder стала условной: строка про обязательный учет OCR добавляется только при наличии runtime `visual_evidence`; при отсутствии скриншотов сохраняется детерминированная секция `Сигналы со скриншотов: Нет приложенных скриншотов.` для trace/debug.
