...
- `v6` (2026-05-04): секция `Сигналы со скриншотов` и OCR-инструкция теперь добавляются только при наличии runtime вложений (`visual_evidence`), чтобы убрать шум в чисто текстовых запросах.

- `v7` (2026-05-07): исправлено два дефекта, приводивших к воспроизведению инжест-артефактов вместо ответа на вопрос.
  1. `_sanitize_context_text()` (`prompt_builder.py`) — удалён bypass `if has_runtime_images: return context_text`. Ранее при наличии скриншотов у пользователя очистка image-чанков отключалась; retriever доставал из Qdrant чанки с сырым VLM chat-диалогом (`[IMAGE]…\nVLM:\nuser\nКратко опиши…\nassistant\n…`), и LLM воспроизводил это описание как ответ. Теперь sanitize применяется всегда. Добавлено второе условие детекции по формату `[IMAGE]…\nVLM:\n` / `[IMAGE]…\nOCR:\n`.
  2. `_collect_chart_signal_hits()` (`main.py`) — сканирование ограничено только сообщениями `role=user`. Ранее функция сканировала все сообщения в истории, включая ответы ассистента: если предыдущий ответ содержал `legend`/`axis`/`chart`, они повторно триггерили `chart_mode` в следующем запросе — замкнутый цикл.
