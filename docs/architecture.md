...
- Возврат текста ответа и списка источников; API также принимает пользовательские скриншоты и возвращает `visual_evidence` (OCR + summary).
...
- Ingestion pipeline A/B:
  ...
  - извлечение изображений из DOCX/PDF, OCR/caption и индексация image-derived чанков;
...
- Vision subsystem:
  - обработка пользовательских скриншотов в `/ask` и `/v1/chat/completions`;
  - OCR (PaddleOCR) + visual summary;
  - структурированные логи этапов `vision_*` в существующий контур Loki/Promtail.
...
- Vision: `Qwen3-VL-2B-Instruct` + `PaddleOCR` для обработки пользовательских скриншотов и изображений из документов.
