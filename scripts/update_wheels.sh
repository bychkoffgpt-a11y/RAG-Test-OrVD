...
download_requirements_with_fallback() {
  ...
  local dest_dir="$2"
  local phase="$3"
  pip_download_with_index_fallback "$phase" "$dest_dir" -r "$req_path"
}

build_requirements_without_torch_stack() {
  local source_req="$1"
  local target_req="$2"
  python3 - "$source_req" "$target_req" <<'PY'
import pathlib
import sys
...
filtered = [line for line in lines if line and not line.startswith(skip_prefixes)]
...
download_cuda_torch_stack() {
  ...
  rm -f "$err_log"
  if [[ -n "$endpoints" ]]; then
    echo "${phase}: не удалось подключиться к PyTorch CUDA index (${endpoints})." >&2
    return 1
  fi
...
validate_wheelhouse() {
  ...
  local req_without_torch_validate
  tmp_validate_dir="$(mktemp -d)"
  req_without_torch_validate="$(mktemp)"
  build_requirements_without_torch_stack "$req_file" "$req_without_torch_validate"

  log "Проверка wheelhouse (фаза A): обычные зависимости через TARGET_PLATFORM=${TARGET_PLATFORM}"
  log "  pip download --no-index --find-links ${wheel_dir} ${TARGET_DOWNLOAD_ARGS[*]} -r ${req_without_torch_validate}"
  if ! python3 -m pip download \
    --disable-pip-version-check \
    ...
    "${TARGET_DOWNLOAD_ARGS[@]}" \
    -r "$req_without_torch_validate"; then
    rm -f "$req_without_torch_validate"
  ...
  log "Проверка wheelhouse (фаза B): torch stack через TORCH_TARGET_PLATFORM=${TORCH_TARGET_PLATFORM}"
  if [[ "$TORCH_TARGET_PLATFORM" == "auto" ]]; then
    log "  TORCH_TARGET_PLATFORM=auto: проверка torch выполняется без --platform/--abi ограничений, как при скачивании"
  fi
  log "  pip download --no-index --find-links ${wheel_dir} ${TORCH_TARGET_DOWNLOAD_ARGS[*]} torch==${TORCH_VERSION} torchvision==${TORCHVISION_VERSION} torchaudio==${TORCHAUDIO_VERSION}"
  if ! python3 -m pip download \
    --disable-pip-version-check \
    --dest "$tmp_validate_dir" \
    --no-index \
...
preflight_check_available_versions() {
  ...
  local req_without_torch
  report_dir="$(mktemp -d)"
  preflight_err_log="$(mktemp)"
  req_without_torch="$(mktemp)"
  build_requirements_without_torch_stack "$req_file" "$req_without_torch"
  local primary_index
  local extra_index
  ...
  log "Preflight (фаза A): проверка обычных зависимостей через общий индекс(ы)"
  if ! download_requirements_with_fallback "$req_without_torch" "$report_dir" "Preflight(A: общий индекс)" 2>"$preflight_err_log"; then
    local endpoints
    ...
    rm -rf "$report_dir"
    if [[ -n "$endpoints" ]]; then
      fail "Preflight (фаза A, общий индекс): не удалось подключиться к ${endpoints} (retries=${PIP_RETRIES}, timeout=${PIP_TIMEOUT}s)."
    fi
    fail "Preflight (фаза A, общий индекс): не удалось скачать обычные зависимости: ${reason}"
  fi
  rm -f "$preflight_err_log"
  ...
  if ! download_cuda_torch_stack "$report_dir" "Preflight(B: PyTorch CUDA index)" 2>"$preflight_err_log"; then
    ...
    fail "Preflight (фаза B, PyTorch CUDA index): не удалось скачать torch stack: ${reason}"
  fi
  rm -f "$preflight_err_log"
  rm -f "$req_without_torch"
...
if [[ "$MODE" == "append" ]]; then
  ...
  if ! validate_wheelhouse "$WHEELS_DIR"; then
    ...
    warn "Wheelhouse после append остаётся неполным/несовместимым (strict отключён)"
  else
    ok "Полнота wheelhouse подтверждена (append)"
  fi
  if [[ "$validation_status" == "ok" ]]; then
    ok "Wheelhouse успешно обновлён и проверен (append): $WHEELS_DIR"
  else
    warn "Wheelhouse обновлён (append), но проверка полноты/совместимости не пройдена: $WHEELS_DIR"
  fi
  exit 0
fi
...
if ! validate_wheelhouse "$tmp_wheels_dir"; then
  ...
  validation_status="warn"
  warn "Скачанный wheelhouse неполный/несовместимый (strict отключён), но каталог будет обновлён."
else
...
if [[ "$validation_status" == "ok" ]]; then
  ...
  warn "Wheelhouse пересобран, но проверка полноты/совместимости не пройдена: $WHEELS_DIR"
fi
